...
def program(h):
	...
	h = {'agentToken':agentToken}
	a= try_command('map enter', h).json()
	global location
	location = a['state']['agents'][agentToken]['locationId']
        metroLine= a['state']['map']['metro']
	print location

	global discredits
	b= try_command('map metro', h).json()
	message =b['action']['message'].split();
	location=message[4]
	discredits -=int( message[8])
	print discredits
	print location

	c= try_command('map bike',h).json()
...
def open_sess(sess_id = ''):
	...
	# if connection successful
		...
		while(usi != 'exit'):
			...
			if usi == 'program': program(h)
			else: try_command(usi, h)
	else:
